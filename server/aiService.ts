...
interface OpenAIClient {
  ...
  images: {
    generate(params: any): Promise<ImageGeneration>;
  };
}
...
export class AIDesignService {
  ...
  async generateStyledRoom(
    originalImageBase64: string,
    roomType: DesignProject['roomType'],
    styleType: DesignProject['styleType']
  ): Promise<string> {
  ...
  private createPrompt(roomType: string, styleType: string): string {
  ...
  private createImagePrompt(roomType: string, styleType: string): string {
    const roomName = roomType.replace("-", " ");
    ...
    return `A beautifully designed ${styleType} ${roomName} interior with ${styleDescriptions[styleType as keyof typeof styleDescriptions] || 'stylish furniture and decor'}. Professional interior design photography, high quality, realistic lighting, 4K resolution.`;
  }

  // Mock function for development - generates a placeholder styled image
  async mockGenerateStyledRoom(
    roomType: DesignProject['roomType'],
    styleType: DesignProject['styleType']
  ): Promise<string> {
    ...
    const roomImages: Record<DesignProject['roomType'], string> = {
